...
def SVMModelDense(train_X, train_Y, pca_test, test_y, lamb, zoom, le_classes_, ngram, kernel=False):
    ...
    for level in xrange(zoom):
        ...
        for i, v in enumerate(lambda_range):
            clf = LogisticRegression(C=v, penalty = 'l2', class_weight=weights, multi_class='ovr', n_jobs=4)
        ...
        sys.stdout.flush()
    clf = clf = LogisticRegression(C=lambda_range[best], penalty = 'l2', class_weight=weights, multi_class='ovr', n_jobs=4)
    model = clf.fit(train_X, train_Y)
    ...
    df.to_csv('predict_proba_logistic_tune_ngram-'+str(ngram)+'.csv', index=False)
